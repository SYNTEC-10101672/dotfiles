...
#### Scenario: sonnet tier 顯示 glm-5-turbo
- **WHEN** `CLAUDE_PROXY_MODE=glm`，`model.display_name` 包含 "sonnet"，`ANTHROPIC_DEFAULT_SONNET_MODEL=glm-5-turbo`
- **THEN** statusline 顯示 `[GLM glm-5-turbo]`

#### Scenario: opus tier 顯示 glm-5.1
- **WHEN** `CLAUDE_PROXY_MODE=glm`，`model.display_name` 包含 "opus"，`ANTHROPIC_DEFAULT_OPUS_MODEL=glm-5.1`
- **THEN** statusline 顯示 `[GLM glm-5.1]`

#### Scenario: haiku tier 顯示 GLM-4.5-Air
...
- **WHEN** `CLAUDE_PROXY_MODE=glm`，`model.display_name` 包含 "glm"（例如 `glm-5-turbo`）
- **THEN** statusline 顯示 `[GLM glm-5-turbo]`
...
- **WHEN** `CLAUDE_PROXY_MODE=glm`，解析結果為 `glm-5-turbo`
- **THEN** statusline 顯示 `[GLM glm-5-turbo] 📁 ...`
