...
`grill-me` 與 `grill-with-docs` 從 `claude/skills/` 遷移至 `claude/commands/` slash commands；此 capability 規範的 skill 集合從 5 個縮為 3 個 model-invoked skills。
...
`grill-me/` 與 `grill-with-docs/` 目錄 SHALL 不存在於 `claude/skills/`（已遷移至 slash commands，見 ADDED requirement）。
...
**Reason**: 此 requirement 規範的 user-invoked skills（`grill-me`、`grill-with-docs`）已遷移至 `claude/commands/`，user-invoked 語意由 slash command 機制保證；保留的 3 個 skills 全部為 model-invoked，分類不再有歧義需規範。
**Migration**: `grill-me.md`、`grill-with-docs.md` commands 的 user-invoked 語意由 `writing-skills` delta 的 MODIFIED requirement（command 機制保證）與本 delta 的 ADDED requirement 涵蓋。
