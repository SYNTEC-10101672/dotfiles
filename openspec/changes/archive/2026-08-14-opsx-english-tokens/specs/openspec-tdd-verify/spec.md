...
### Requirement: 驗證區塊格式
tasks.md 中的驗收條件 SHALL 以獨立 T* 測試項目呈現（`## Tests` 區塊），而非 inline `> 驗證：` blockquote。T* 項目包含 `> Command:` 與 `> Expected:` 兩個結構化欄位。
...
- **THEN** skill 讀取 `## Tests` 區塊中的 T* 項目，提取 `> Command:` 執行，並以 `> Expected:` 判斷結果
...
- **THEN** 執行所有 T* 的 `> Command:`，預期結果與 `> Expected:` 不符（fail），確認測試尚未通過；若某個 T* 在 Red phase 已通過，回報並詢問使用者是否繼續

#### Scenario: Green phase——實作後確認測試 pass
- **WHEN** 對應的實作 task 完成後，呼叫 tdd-verify 執行對應 T*
- **THEN** 執行該 T* 的 `> Command:`，結果符合 `> Expected:`（pass），mark T* `[x]`

#### Scenario: 驗證失敗
- **WHEN** 執行 T* 的 `> Command:` 後結果不符合 `> Expected:`
...
#### Scenario: AI 可直接執行驗證
- **WHEN** T* 的 `> Command:` 是明確的 shell 指令
...
- **THEN** skill 輸出 `⚠ Manual verification: <描述使用者需確認的項目>`，mark T* `[x]`（帶注記），繼續下一個 T*
