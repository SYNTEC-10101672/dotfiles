...
**Reason**: 使用者實際進入點一律為 slash commands，thin wrapper 造成每次啟動多一次 skill tool 跳轉與雙檔維護；entry skill 內容改由 commands 直接承載（merge-opsx-commands 變更）。
**Migration**: wrapper 的 frontmatter、用途說明、Input 段保留於 command 檔案開頭，workflow 內容自對應 skill body 逐字合併；「Use the `openspec-<name>` skill」指向行刪除。新結構見 ADDED「核心 opsx commands SHALL 為內容自足的 workflow 本體」。

### Requirement: Skill 檔案 SHALL 為 workflow 內容的單一真相來源
**Reason**: 真相來源位置由 skill 檔移至 command 檔；requirement 名稱與內容以舊位置綁定，隨位置改變由新 requirement 取代。
**Migration**: 4 個 entry skill 目錄刪除；對應內容由 `opencode/commands/opsx/<name>.md` 單一檔案承載（見 ADDED「Command 檔案 SHALL 為 workflow 內容的單一真相來源」）。

### Requirement: 保留的 skills SHALL 無 dangling cross-references
**Reason**: 約束對象由 skills 擴及合併後的 commands 與 helper skills 全集合，且新增「舊流程名識別字不得殘留」規則；原 requirement 僅涵蓋 skills。
**Migration**: 見 ADDED「保留的 opsx 檔案 SHALL 無 dangling cross-references」。
...
- **THEN** 只含 `openspec-tdd-verify`、`openspec-code-review`、`openspec-artifact-review`、`openspec-sync-specs`

### Requirement: 保留的 opsx 檔案 SHALL 無 dangling cross-references

保留的 command 檔案與 helper skill 檔案內所有 cross-reference（skill 名、slash command 名、流程名）SHALL 指向存在的檔案或流程。entry skill 名識別字——`openspec-explore`、`openspec-propose`、`openspec-apply-change`、`openspec-archive-change`、以及 bare `openspec-apply`——SHALL NOT 出現於 `opencode/` 內任何檔案；流程引用一律使用 slash form（`/opsx:explore`、`/opsx:propose`、`/opsx:apply`、`/opsx:archive`）。

#### Scenario: Helper skill description 引用 slash form

- **WHEN** 檢視 `opencode/skills/openspec-artifact-review/SKILL.md` 與 `opencode/skills/openspec-code-review/SKILL.md` 的 frontmatter description
...
只保留實際使用的 workflow 檔案：4 個核心 commands（`explore.md` / `propose.md` / `apply.md` / `archive.md`，各為內容自足的 workflow 本體）+ 4 個 helper skills（`openspec-tdd-verify` / `openspec-code-review` / `openspec-sync-specs` / `openspec-artifact-review`）。`openspec-explore` / `openspec-propose` / `openspec-apply-change` / `openspec-archive-change` skill 目錄 SHALL 不存在於 `opencode/skills/`。
...
- **WHEN** 列出 `opencode/skills/` 中 `openspec-` 開頭目錄
- **THEN** 只含 `openspec-tdd-verify`、`openspec-code-review`、`openspec-sync-specs`、`openspec-artifact-review`
