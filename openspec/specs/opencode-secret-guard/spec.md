# Spec: Opencode Secret Guard

## Purpose

Define requirements for the OpenCode-layer secret leak protection (Secret Guard): `permission.read` deny rules for sensitive files, `guard.ts` staged scan before `git commit`, fail-closed behavior when gitleaks is missing, and human confirmation for `git push`.
