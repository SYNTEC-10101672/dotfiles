...
`openspec-propose` 生成 `tasks.md` 時，MUST 同時產出 `## Tests` section 的 T\* 測試項目（T1、T2…，各含 Command / Expected），使驗收契約隨所有 artifacts 一併接受 momus gate 與人工 review。docs-only change 若無可執行驗證，MUST 在 tasks.md 記載豁免理由。
...
- **WHEN** propose 進入 momus gate 與人工 review
