...
Claude Code hook 驅動的 tmux window bar 通知機制。任務完成時顯示綠色 `✓`，等待使用者回應時顯示綠色 `?`，切換到該 window 後自動消失，不修改 window 名稱。
...
### Requirement: 指示符自動消失
使用者切換到已通知的 window 時，指示符 SHALL 自動消失。

#### Scenario: 切換視窗時自動清除
- **WHEN** 使用者切換到帶有 `@claude_state` 的 window
- **THEN** tmux `after-select-window` hook 清除該 window 的 `@claude_state`
- **THEN** window bar 不再顯示 `✓` 或 `?`
