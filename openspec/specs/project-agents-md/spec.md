# project-agents-md Specification

## Purpose

規範 dotfiles repo 的 project-level 指示檔（repo root `AGENTS.md`）：存在性、內容大綱、與全域指示檔 `opencode/AGENTS.md` 的分工。承接原 `project-claude-md` capability 的職責（路徑與載入機構隨 migrate-config-to-opencode change 更新）。
## Requirements
